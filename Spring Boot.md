...
想让Spring的配置文件生效，加载进来；@**ImportResource**标注在一个配置类上，视频的例子是标准在了main的主类上
...
### 1、随机数（*这些应该都是写在properties或yml文件中的参数）
...
person.maps.k2=14
person.lists=a,b,c
person.dog.name=${person.hello:hello}_dog	#这里冒号后面是，当没有person.hello没有定义时候的默认值
person.dog.age=15
```
...
java -jar spring-boot-02-config-02-0.0.1-SNA erties
...
配置文件到底能写什么？怎么写？自动配置原理；（*properties大列表） 
...
**2）、@EnableAutoConfiguration 作用：**
 ...
 -  利用EnableAutoConfigurationImportSelector给容器中导入一些组件？（*@Import）
...
public class ThymeleafProperties {	//thymeleaf默认规则
...
1、导入thymeleaf的名称空间（语法提示，不导应该也能用）
...
- 自动注册了 of `Converter`, `GenericConverter`, `Formatter` beans.
  ...
  - Converter：转换器；  public String hello(User user)：类型转换使用Converter，类似"18"==>18
  - `Formatter`  格式化器；  2017.12.17===Date；

```java
		@Bean（如果配置了日期格式化转换器就注册，如果没配，就不注册了）
...
- Support for `HttpMessageConverters` (see below).
  ...
  - HttpMessageConverter：SpringMVC用来转换Http请求和响应的；User--->Json；消息转换器
...
- Automatic use of a `ConfigurableWebBindingInitializer` bean (see below).
  ...
  ```
  初始化WebDataBinder；数据绑定器，还会涉及到类型转化格式化等前面的组件
  请求数据=====JavaBean；请求数据绑定到javabean中
  ```
...
```xml
    <mvc:view-controller path="/hello" view-name="success"/>	视图映射，hello映射到success
    <mvc:interceptors> mvc拦截器
        <mvc:interceptor>
            <mvc:mapping path="/hello"/>	拦截hello请求，到bean下
            <bean></bean>
        </mvc:interceptor>
    </mvc:interceptors>
```
...
这里可以配置配置文件spring.message.basename
